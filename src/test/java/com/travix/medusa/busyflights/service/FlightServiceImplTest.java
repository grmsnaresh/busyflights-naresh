...
public class FlightServiceImplTest extends SpringBaseTest {
	...
	static final Flight FLIGHT_1 = new Flight(Airline.KLM, Supplier.CRAZY_AIR, CRAZY_AIR_PRICE,
			AirportCodes.CAIRO_AIRPORT, AirportCodes.AMSTERDAM_AIRPORT_SCHIPHOL,
	...
	@Before
	public void setup() {
		when(crazyAirSupplierService.searchFlights(nullable(FlightSearch.class))).
			thenReturn(Arrays.asList(FLIGHT_1));

		when(toughJetSupplierService.searchFlights(nullable(FlightSearch.class))).
